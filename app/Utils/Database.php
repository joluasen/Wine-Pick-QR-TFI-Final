...
class Database
{
    ...
    $sslCa = defined('DB_SSL_CA') ? trim((string) DB_SSL_CA) : '';
    $useSsl = defined('DB_SSL_VERIFY') ? (bool) DB_SSL_VERIFY : false;
    ...
    if ($useSsl || $sslCa !== '') {
        ...
        $caPath = ($sslCa !== '') ? $sslCa : null;
        $mysqli->ssl_set(null, null, $caPath, null, null);
        $clientFlags = MYSQLI_CLIENT_SSL;
    }

    $ok = @$mysqli->real_connect($host, $user, $pass, $name, $port, null, $clientFlags);

    if (!$ok || $mysqli->connect_errno) {
        throw new \Exception('Error de conexión a BD: ' . $mysqli->connect_error);
    }
